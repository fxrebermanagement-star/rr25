...
function paintLog(){
  ...
  box.innerHTML=rows.length?rows.map(e=>`<div class="card"><button type="button" class="card" data-open="${e.id}" style="margin:0;border:0;padding:0;background:transparent"><b>${esc(e.titel)}</b><small>${esc(e.t)} ${esc(e.wer||"")} ${e.wesen?"· Wesenheit":""}</small></button>${e.note?`<p class="sub" style="text-align:left;margin:.4rem 0 0">${esc(e.note)}</p>`:""}<input data-note="${e.id}" placeholder="Kommentar für später" value="${escAttr(e.note||"")}"></div>`).join(""):"<p class='sub'>Noch leer.</p>";
